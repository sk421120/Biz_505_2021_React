...
const headArray = ["DATE", "TIME", "REMEMBER"];
// const rem_header = () => {
...
function RemList() {
  /*
	...
	함수이름을 통해서 마치 한개의 명령문처럼 코드를 사용하는데 목적이 있다
	...
	react는 한번 rendering된 화면은 특별한 값(상태)의 변화가 발생하면 그 부분만
	다시 그리는 동작이 발생한다

	어떤 화면(컴포넌트)이 하나 생성되었는데 그 화면을 다시 열고, 닫는 동작이 반복될때
	내부의 코드는 변함이 없을 것이다
  ...
  /*
	...
	useCallback()으로 감싸진 함수는 이전에 한번이라도 만들어진 경우는
	그대로 재사용하고 없는 경우에만 함수를 생성한다
	*/
  const fetchCallback = useCallback(() => {
    const remString = localStorage.listOfRem;
    if (remString) {
      console.log("Fetch rememberList");
      console.log(remString);
      const remJSON = JSON.parse(remString);
      console.table(remJSON);
      setRememberList(remJSON);
    }
  }, []);

  // 상태가 없으면 최초에 rendering 될 때 한번만 함수를 호출한다
  // 변화를 감시할 상태 데이터가 없으면
  // 	최초에 rendering(app 시작될때, page가 열릴때, 새로고침) 될때 fetchData를 실행한다
  useEffect(fetchCallback, [fetchCallback]);

  const saveStorage = () => {
    // console.log("EFFECT");
    /*
	rememberList 객체 배열에 담긴 데이터를 JSON String 문자열로 변환하여
	JSON.stringify() localStorage에 RememberList 라는 이름으로 저장하라
	*/
    if (rememberList.length > 0) {
      ...
      localStorage.listOfRem = JSON.stringify(rememberList);
    }
  };
  ...
  /*
  list중 한 요소를 더블클릭하면 선택된 요소의 UUID값을 추출하여
  1. 해당 요소를 삭제하기
